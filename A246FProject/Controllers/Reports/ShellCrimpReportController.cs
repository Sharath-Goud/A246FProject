...
{
    public class ShellCrimpReportController : Controller
    {
        private readonly ShellCrimpReportBAL _bal;
        private readonly MasterBAL _master;

        public ShellCrimpReportController()
        {
            _bal = new ShellCrimpReportBAL();
        ...
        {
            ShellCrimpReportViewModel model = new();
            ...
            return View("~/Views/Reports/ShellCrimpReport.cshtml", model);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Index(ShellCrimpReportViewModel model, string command)
        {
            model.Lines = _master.GetLine();
            model.Shifts = _master.GetShift();
            model.Projects = _master.GetProject();
            ...
            if (command == "Search")
            {
                ...
                model.dtReports = _bal.GetShellCrimpReport(
                    date,
                    model.LineId,
                    model.ShiftId,
                    model.ProjectId,
