...
{
    public class LaserEngravingReportController : Controller
    {
        private readonly LaserEngravingReportBAL _bal;
        private readonly MasterBAL _master;

        public LaserEngravingReportController()
        {
            _bal = new LaserEngravingReportBAL();
        ...
        {
            LaserEngravingReportViewModel model = new();
            ...
            return View("~/Views/Reports/LaserEngravingReport.cshtml", model);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Index(LaserEngravingReportViewModel model, string command)
        {
            model.Lines = _master.GetLine();
            model.Shifts = _master.GetShift();
            model.Projects = _master.GetProject();
            ...
            if (command == "Search")
            {
                ...
                model.dtReports = _bal.GetLaserEngravingReport(
                    date,
                    model.LineId,
                    model.ShiftId,
                    model.ProjectId);
