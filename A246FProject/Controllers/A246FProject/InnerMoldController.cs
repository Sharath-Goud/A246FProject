...
{
    public class InnerMoldController : Controller
    {
        InnerMoldBAL _bal =
            new InnerMoldBAL();
        ...
        [HttpPost]
        public IActionResult SaveA246FInnerMoldCheckList([FromBody] InnerMoldViewModel model)
        {
            try
            {
                if (model?.InnerMoldResults == null)
                ...
                foreach (var row in model.InnerMoldResults)
                {
                    dt.Rows.Add(
                        i++,
                        row.SectionId,
